...
function updateTab(key, cronstring) {
  try {
    ...
    if (typeof cronstring === 'string' || cronstring instanceof Date) {
        var preservedOptions = getAdditionalOptions(key)
...
function getAdditionalOptions(key) {
    ...
    function getOptionsFromCronTime() {
        if (job.cronTime.hasOwnProperty('timeZone')) additonalOptions.timeZone =job.cronTime.timeZone
        if (job.cronTime.hasOwnProperty('utcOffset')) additionalOptions.utcOffset = job.crontTime.utcOffset
    }
